...
public class BasicHybridTest {
        ...
        // Docker image that contains a running Spring-instance that talks to the Genapp
        // webservice
        @DockerContainer(image = "lukasmarivoet/genapp-spring:latest")
        public IDockerContainer container;

        @Test
        public void basicHybridTest()
                        throws CoreManagerException, Zos3270Exception, TestBundleResourceException, JsonSyntaxException,
                        IOException, HttpClientException, URISyntaxException, DockerManagerException {
                ...
                // Adding an empty customer to the data so that we can retrieve its customer
                // number
                JsonObject addResponseBody = client
                                .postJson("addcustomer?host=" + genApp.getBaseAddress(), new JsonObject()).getContent();
                String customerId = addResponseBody.get("content").getAsString();

                // Inquiring the specific customer through Spring that in turn calls towards the
                // Genapp Web based instance
                JsonObject inquireResponseBody = client
                                .postJson("inquirecustomer?host=" + genApp.getBaseAddress() + "&id=" + customerId,
                                                new JsonObject())
                                .getContent();
                ...
                JsonObject inquireAfterUpdateResponseBody = client
                                .postJson("inquirecustomer?host=" + genApp.getBaseAddress() + "&id=" + customerId,
                                                new JsonObject())
                                .getContent();
                JsonObject caAfterUpdate = inquireAfterUpdateResponseBody.get("LGICUS01OperationResponse")
                                .getAsJsonObject().get("ca").getAsJsonObject();
        ...
        private void inquireCustomer(String customerId) throws TimeoutException, KeyboardLockedException,
